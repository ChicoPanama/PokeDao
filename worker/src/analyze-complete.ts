...
function analyzeCompleteDataset() {
  try {
    ...
    const investmentCandidates = cardsWithPrices.filter(card => {
      const listing = card.listing || {}
      const price = listing.price || 0
    ...
    // Price distribution
    const prices = investmentCandidates.map(card => {
      const listing = card.listing || {}
      const price = listing.price || 0
      const currency = listing.currency || "USDC"
    ...
    // Investment tiers
    const tiers = {
      "Budget ($15-$100)": prices.filter(p => p >= 15 && p < 100).length,
      "Entry ($100-$300)": prices.filter(p => p >= 100 && p < 300).length,
      "Mid ($300-$1000)": prices.filter(p => p >= 300 && p < 1000).length,
      "High ($1000-$5000)": prices.filter(p => p >= 1000 && p < 5000).length,
      "Premium ($5000+)": prices.filter(p => p >= 5000).length
    }
