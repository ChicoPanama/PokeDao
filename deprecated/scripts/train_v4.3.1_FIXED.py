...
# Fixed formatting function that handles both single and batched examples
def formatting_func(example):
    # SFTTrainer may pass single example or batch, handle both
    messages = example['messages']
    if isinstance(messages, list) and len(messages) > 0:
        if isinstance(messages[0], dict):
            # Single example: messages is [{"role": "user", "content": "..."}, ...]
            user_msg = messages[0]['content']
            assistant_msg = messages[1]['content']
        else:
            # Batched example: messages is [[{"role": "user", ...}], [{"role": "user", ...}]]
            user_msg = messages[0][0]['content']
            assistant_msg = messages[0][1]['content']
    else:
