/**
 * Magic Eden Phygitals Harvester
 * Since Phygitals is on-chain (Solana), Magic Eden has the data
 */
...
class MagicEdenPhygitalsHarvester {
  constructor() {
    this.baseURL = 'https://api-mainnet.magiceden.io';
    this.db = new Database('magic_eden_phygitals.db');
    this.setupDatabase();
    ...
    console.log('🎯 Target: All Phygitals Pokemon cards on Solana');
    console.log('📅 Session:', this.sessionId);
  }

  setupDatabase() {
    console.log('🗃️ Setting up Magic Eden database schema...');
    
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS magic_eden_phygitals (
        id TEXT PRIMARY KEY,
      ...
      CREATE INDEX IF NOT EXISTS idx_me_name ON magic_eden_phygitals(name);
      CREATE INDEX IF NOT EXISTS idx_me_price ON magic_eden_phygitals(usd_price);
      CREATE INDEX IF NOT EXISTS idx_me_pokemon ON magic_eden_phygitals(pokemon_name);
      CREATE INDEX IF NOT EXISTS idx_me_grader ON magic_eden_phygitals(grader, grade);
      CREATE INDEX IF NOT EXISTS idx_me_set ON magic_eden_phygitals(set_name);
      CREATE INDEX IF NOT EXISTS idx_me_collection ON magic_eden_phygitals(collection);
      CREATE INDEX IF NOT EXISTS idx_me_mint ON magic_eden_phygitals(mint_address);
      CREATE INDEX IF NOT EXISTS idx_me_session ON magic_eden_phygitals(harvest_session);
      
      -- Sales tracking table
      CREATE TABLE IF NOT EXISTS magic_eden_sales (
        id TEXT PRIMARY KEY,
        mint_address TEXT,
        sale_price REAL,
        ...
        harvest_session TEXT,
        harvest_timestamp TEXT DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (mint_address) REFERENCES magic_eden_phygitals(mint_address)
      );
      
      -- Collections tracking
      CREATE TABLE IF NOT EXISTS magic_eden_collections (
    ...
    console.log('✅ Magic Eden database setup complete');
  }
  ...
  async discoverPhygitalsCollections() {
    console.log('🔍 DISCOVERING PHYGITALS COLLECTIONS ON MAGIC EDEN');
    console.log('==================================================');

    try {
      // Search for Phygitals collections
      const searchUrl = `${this.baseURL}/v2/collections?offset=0&limit=100&sortBy=volume&sortDirection=desc`;
  ...
  async harvestCollection(collection) {
    ...
    try {
      ...
      const statsUrl = `${this.baseURL}/v2/collections/${collection.symbol}/stats`;
      ...
      const collectionStmt = this.db.prepare(`
        INSERT OR REPLACE INTO magic_eden_collections 
      ...
      while (offset < 10000) { // Safety limit
        try {
          const tokensUrl = `${this.baseURL}/v2/collections/${collection.symbol}/tokens?offset=${offset}&limit=${limit}`;
  ...
  async processAndStoreToken(token, collection) {
    try {
      ...
      const tokenData = [
        ...
        // Source tracking
        `https://magiceden.io/item-details/${token.mintAddress}`,
        `https://magiceden.io/item-details/${token.mintAddress}`,
        this.sessionId,
        new Date().toISOString(),
      ...
      const insertStmt = this.db.prepare(`
        INSERT OR REPLACE INTO magic_eden_phygitals (
          ...
          source_url, magic_eden_url, harvest_session, harvest_timestamp,
  ...
  async generateReport() {
    console.log('\n📊 GENERATING MAGIC EDEN HARVEST REPORT');
    console.log('=======================================');

    const stats = {
      totalTokens: this.db.prepare('SELECT COUNT(*) as count FROM magic_eden_phygitals').get().count,
      tokensWithPrices: this.db.prepare('SELECT COUNT(*) as count FROM magic_eden_phygitals WHERE usd_price > 0').get().count,
      pokemonTokens: this.db.prepare('SELECT COUNT(*) as count FROM magic_eden_phygitals WHERE pokemon_name IS NOT NULL').get().count,
      gradedTokens: this.db.prepare('SELECT COUNT(*) as count FROM magic_eden_phygitals WHERE is_graded = 1').get().count,
      vintageTokens: this.db.prepare('SELECT COUNT(*) as count FROM magic_eden_phygitals WHERE is_vintage = 1').get().count,
      highValueTokens: this.db.prepare('SELECT COUNT(*) as count FROM magic_eden_phygitals WHERE usd_price > 100').get().count
    };
    ...
    fs.writeFileSync('magic-eden-phygitals-harvest.json', JSON.stringify(report, null, 2));
    console.log(`\n📄 Report saved: magic-eden-phygitals-harvest.json`);

    return report;
  }

  async runHarvest() {
    console.log('\n🚀 STARTING MAGIC EDEN PHYGITALS HARVEST');
    console.log('=========================================');
    ...
    try {
      ...
      if (collections.length === 0) {
        console.log('⚠️ No Phygitals collections found on Magic Eden');
      ...
      console.log(`\n🎉 SUCCESS! Magic Eden harvest complete!`);
      console.log(`📊 Total tokens processed: ${this.totalProcessed}`);
      
      return { success: true, report };

    } catch (error) {
      console.error('💥 Magic Eden harvest failed:', error);
...
// Execute the harvest
async function main() {
  const harvester = new MagicEdenPhygitalsHarvester();
...
module.exports = MagicEdenPhygitalsHarvester;
