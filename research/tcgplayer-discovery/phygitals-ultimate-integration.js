...
class PhygitalsIntegrationSystem {
  ...
  integrateWithUltimatePricing() {
    ...
    try {
      const ultimateCount = this.ultimateDb.prepare('SELECT COUNT(*) as count FROM collector_crypt_ultimate_pricing').get().count;
    ...
    try {
      this.ultimateDb.exec(`ALTER TABLE collector_crypt_ultimate_pricing ADD COLUMN phygitals_price REAL`);
      this.ultimateDb.exec(`ALTER TABLE collector_crypt_ultimate_pricing ADD COLUMN phygitals_source TEXT`);
  ...
  findCardMatches(phygitalsCard) {
    ...
    let matches = this.ultimateDb.prepare(`
      SELECT id, cc_title as name FROM collector_crypt_ultimate_pricing
  ...
  enhanceUltimatePricingWithPhygitals() {
    ...
    const cardsWithPhygitals = this.ultimateDb.prepare(`
      SELECT 
        id, cc_title as name, ebay_sold_price as ebay_sold_avg, enhanced_pokemon_price as enhanced_pokemon_tcg_price, 
        phygitals_price, final_market_value as ultimate_market_value, final_confidence as confidence_score
    ...
    for (const card of cardsWithPhygitals) {
      ...
      if (prices.length >= 2) {
        ...
        const updateStmt = this.ultimateDb.prepare(`
          UPDATE collector_crypt_ultimate_pricing 
          SET final_market_value = ?, final_confidence = ?
          WHERE id = ?
        `);
  ...
  generateIntegrationReport() {
    ...
    const totalCards = this.ultimateDb.prepare('SELECT COUNT(*) as count FROM collector_crypt_ultimate_pricing').get().count;
    const cardsWithPhygitals = this.ultimateDb.prepare('SELECT COUNT(*) as count FROM collector_crypt_ultimate_pricing WHERE phygitals_price IS NOT NULL').get().count;
    
    const priceComparison = this.ultimateDb.prepare(`
      SELECT 
        AVG(final_market_value) as avg_ultimate_price,
        AVG(phygitals_price) as avg_phygitals_price,
        AVG(ebay_sold_price) as avg_ebay_price
      FROM collector_crypt_ultimate_pricing 
      WHERE phygitals_price IS NOT NULL AND final_market_value > 0
    `).get();

    const topPhygitalsDeals = this.ultimateDb.prepare(`
      SELECT 
        cc_title as name, final_market_value as ultimate_market_value, phygitals_price, ebay_sold_price as ebay_sold_avg,
        (final_market_value - phygitals_price) as price_difference
      FROM collector_crypt_ultimate_pricing 
      WHERE phygitals_price IS NOT NULL 
        AND final_market_value > phygitals_price
        AND final_market_value > 10
