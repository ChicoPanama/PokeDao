"""
MEW-1A v4.3-SHAPED vLLM DEPLOYMENT WITH RESPONSE SHAPING
==========================================================

Production deployment of Mew-1A v4.3 with Vector RAG + Phase 1 Contradiction Elimination.
...
- Response Shaping Layer (eliminates visible contradictions)
- Policy-Aligned Headlines ("RECOMMENDATION: BUY/PASS/HOLD")
- Contradiction Sanitization (removes conflicting BUY/PASS terms)
- Template Rebuild for Fallback Cases (HOLD/NEUTRAL)
- New Metrics: visible_contradictions_total, hold_fallback_total, response_sanitized_total

Target SLOs:
- Visible Contradictions: 0%
- Rules Baseline Agreement: 100% (Policy Engine enforced)
- Fallback Rate: <5%
- p95 Latency: <10s
...
# Response Shaping Configuration (Phase 1: Contradiction Elimination)
FORCE_POLICY_HEADLINE = os.environ.get("FORCE_POLICY_HEADLINE", "true").lower() == "true"

# Modal App (separate namespace for v4.3-shaped canary)
app = modal.App("mew1a-vllm-v4-3-shaped")
...
def fastapi_app():
    ...
    async def generate(request: Request):
        ...
        try:
            ...
            shaped_response = {}
            if policy["policy_engine"] and listed_price is not None and fair_value is not None and listed_price > 0 and fair_value > 0:
                from response_shaper import shape_response
                ...
                pm.record_response_shaping("/generate", shaped["was_sanitized"], shaped["was_rebuilt"])

                # Record visible contradiction if it still exists (should never happen)
                if shaped["visible_contradiction"]:
                    pm.record_visible_contradiction("/generate")
            else:
